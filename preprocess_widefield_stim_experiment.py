...
skio.imsave(os.path.join(output_folder, "stim_frames_removed", "%s.tif" % expt_name), np.round(stim_frames_removed).astype(np.uint16))
...
for subfolder in ["stim_frames_removed", "denoised", "corrected"]:
    ...
    with open(os.path.join(output_folder, "%s/analysis/stim_indices/%s_stim_indices.pickle" % (subfolder, expt_name)), "wb") as f:
