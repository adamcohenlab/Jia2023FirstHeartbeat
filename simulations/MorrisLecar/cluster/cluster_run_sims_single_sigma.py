...
if args.from_state:
    ...
    for i in range(len(Is)-1):
        I_min = Is[i]
        I_max = Is[i+1]
        cmd = ["sbatch", str(SPIKECOUNTER_PATH/"simulations/MorrisLecar/cluster/batch_sim.sh"), rootpath, bifurcation, str(I_min),\
               str(I_max), str(args.sigma), str(int(total_blocks/n_jobs)),\
