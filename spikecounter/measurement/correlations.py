...
def spearman_image_to_trace(img, trace):
    correlation = np.zeros((img.shape[1], img.shape[2], img.shape[3]))
    for z in range(img.shape[1]):
        for y in range(img.shape[2]):
            for x in range(img.shape[3]):
                correlation[z, y, x] = stats.spearmanr(img[:, z, y, x].astype(float), trace.astype(float))[0]
                if np.isnan(correlation[z,y,x]):
                    correlation[z, y, x] = 0
    return correlation

def pearson_image_to_trace(img, trace):
    correlation = np.zeros((img.shape[1], img.shape[2], img.shape[3]))
    for z in range(img.shape[1]):
        for y in range(img.shape[2]):
            for x in range(img.shape[3]):
                correlation[z, y, x] = stats.pearsonr(img[:, z, y, x].astype(float), trace.astype(float))[0]
                if np.isnan(correlation[z,y,x]):
                    correlation[z, y, x] = 0
    return correlation
    
def dot_image_to_trace(img, trace):
    correlation = np.zeros((img.shape[1], img.shape[2], img.shape[3]))
