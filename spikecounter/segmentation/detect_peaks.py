...
def detect_peaks(x, mph=None, mpd=1, threshold=0, relative_threshold=1, edge='rising', offset_threshold = 0, max_half_width=3, 
    ...
    if ind.size:
        ind = ind[ind > max_half_width]
        ind = ind[ind < x.size - max_half_width]
    ...
    if ind.size and threshold > 0:
        dx = np.max(np.vstack([x[ind]-x[ind-max_half_width], x[ind]-x[ind+max_half_width]]), axis=0)
